...
1. Edit `helper-config.json` to change league settings, rest-of-season projection source URLs, or manual player overrides.
2. Run:
...
Backfilled weekly points use the current projection internally to estimate overall score plus reconstructed season and recent-game form through the prior Sunday. For dates before the first real projection snapshot, the Projection trend uses a Razzball/Steamer preseason baseline so the early-season chart has a meaningful reference line. If a later backfilled week is missing a real projection snapshot, the Projection trend carries forward the most recent real projection snapshot. Savant trend points are only captured on normal data refreshes.
